...
export function AgentPromptQueue({items,paused,locale='en',disabled=false,onEdit,onRemove,onReorder,onPause,onRetry}:{
  ...
  return <section className="max-h-48 overflow-y-auto rounded-xl border border-border bg-background p-2 text-xs" data-xgc-role="agent-prompt-queue" aria-label={zh?'待发送消息':'Queued messages'}>
    <div className="flex items-center justify-between gap-2 px-1 pb-1"><span>{paused?(zh?'已暂停':'Paused'):(zh?'待发送':'Queued')}</span>
      {items.some(p=>!p.local)?<Button size="icon-xs" variant="ghost" disabled={disabled||busy} aria-label={paused?(zh?'继续发送队列':'Resume queue'):(zh?'暂停队列':'Pause queue')} data-xgc-role="agent-queue-pause" onClick={()=>void act(()=>onPause(!paused))}>{paused?<Play className="size-3"/>:<Pause className="size-3"/>}</Button>:null}
    </div>
    {items.map((item,index)=><div key={item.id} className="border-t border-border py-1" data-xgc-role="agent-queued-message" data-xgc-id={item.id}
      ...
      <div className="flex items-center gap-1">
        <button type="button" draggable={!disabled&&!busy&&!item.local} disabled={disabled||busy||item.local} aria-label={zh?'拖动排序':'Drag to reorder'} data-xgc-role="agent-queue-drag" data-xgc-id={item.id} className="cursor-grab p-1 text-muted-foreground disabled:opacity-30"
