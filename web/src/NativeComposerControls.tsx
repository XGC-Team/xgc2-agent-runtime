...
import type { NativeTurnOptions } from './state.js'
...
export type NativeComposerSelection = NativeTurnOptions & { profileId: string }
export type NativeComposerControlsProps = {
  identityId?: string
  locale?: NativeLocale
  providers: readonly NativeProviderConfiguration[]
  value: NativeComposerSelection
  onChange: (selection: NativeComposerSelection) => void | Promise<unknown>
  disabled?: boolean
  disabledReason?: string
  active?: boolean
}
/** A draft selection only. The host submits these exact options with the next native turn. */
export function NativeComposerControls({ providers, value, onChange, disabled = false, disabledReason = '', active = true, locale = 'en', identityId }: NativeComposerControlsProps) {
  ...
  const change = async (next: NativeComposerSelection) => {
  ...
  return <div className="w-full min-w-0" hidden={!active} data-xgc-role="native-composer-controls" data-xgc-id={identityId ?? (value.profileId || 'unselected')}>
    <TooltipProvider>
      <div className="flex w-full min-w-0 flex-nowrap items-center gap-1" data-xgc-composer-row="">
        {hasModelChoices && (!selected || selected.models.length > 0) ? <ProviderModelPicker identityId={identityId} locale={locale} providers={providers} profileId={value.profileId} model={value.model} disabled={locked || selected?.enabled === false} onChange={(profileId, modelId) => {
          ...
          const next: NativeComposerSelection = { profileId, model: modelId }
          ...
          data-xgc-role="native-composer-model" data-xgc-id={modelIdentity}>
          {ProviderIcon ? <ProviderIcon className="size-4 shrink-0" aria-hidden="true" /> : null}
          <span className="min-w-0 truncate">{model?.label ?? value.model ?? selected?.defaults.model ?? (locale === 'zh' ? '原生默认模型' : 'Native default model')}</span>
        </ComposerControl>}
        {selected && model?.efforts.length ? <ComposerEffortControl identityId={identityId} locale={locale} profileId={selected.id} options={model.efforts} value={effort} disabled={providerLocked} onChange={effort => void change({ ...value, effort })} /> : <ComposerControl type="button" disabled className="shrink-0 justify-start whitespace-nowrap"
          ...
          data-xgc-role="native-composer-effort" data-xgc-id={modelIdentity}>
        ...
        {selected && selected.permissions.length ? <ComposerPermissionControl identityId={identityId} locale={locale} profileId={selected.id} options={selected.permissions} value={permission} disabled={providerLocked} onChange={permission => void change({ ...value, permission })} /> : <ComposerControl type="button" disabled className="min-w-0 shrink-0 font-medium"
          ...
          data-xgc-role="native-composer-permission" data-xgc-id={modelIdentity}>
