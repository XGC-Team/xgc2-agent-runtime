...
export function NativeToolImages({ images, identity }: { images: NonNullable<ToolData['images']>; identity: string }) {
  return <div className="ms-7 mt-2 flex flex-col gap-2">
    {images.map((image, index) => <figure key={`${identity}:${index}`} className="m-0 min-w-0" data-xgc-role="native-agent-tool-image" data-xgc-id={`${identity}:${index}`}>
      <img src={image.src} alt={image.label} width={image.width} height={image.height}
        ...
        data-xgc-role="native-agent-tool-thumbnail" data-xgc-id={`${identity}:${index}`} />
