export const NATIVE_SCHEMA = 'xgc.native-agent/v1' as const
...
export type NativeCommandAction = { type: 'read' | 'listFiles' | 'search' | 'unknown'; command: string; name?: string; path?: string; query?: string }
export type NativeFileChange = { path: string; kind: { type: 'add' | 'delete' | 'update'; movePath?: string }; diff: string }
export type NativeToolContent = { type: 'text'; text: string } | { type: 'resource_link'; uri: string; name?: string; mimeType?: string; description?: string } | { type: 'image'; mimeType: 'image/jpeg' | 'image/png'; data: string; width: number; height: number }
export type NativeItemDetails = (
  | { type: 'userMessage'; nativeOptions: NativeTurnOptions }
  | { type: 'agentMessage'; phase?: 'commentary' | 'final_answer' }
  | { type: 'commandExecution'; command: string; cwd?: string; exitCode?: number; durationMs?: number; commandActions?: NativeCommandAction[] }
  | { type: 'fileChange'; changes: NativeFileChange[] }
  | { type: 'mcpToolCall'; server: string; tool: string; arguments?: NativeJson; result?: { content: NativeToolContent[]; structuredContent?: NativeJson }; error?: { message: string }; durationMs?: number }
...
export type NativeRequestDetails = { command?: string; cwd?: string; reason?: string; grantRoot?: string; network?: { protocol: string; host: string }; toolName?: string; target?: string; preview?: string; truncated?: boolean }
export type DecisionOption = { id: string; label: string; kind: string; description?: string }
export type NativeQuestion = { id: string; text: string; options: DecisionOption[]; multiple: boolean; freeText: boolean; header?: string }
export type NativeRequest = { id: string; kind: 'permission' | 'question' | 'plan'; title: string; text?: string; options: DecisionOption[]; questions: NativeQuestion[]; sourceMethod?: string; nativeThreadId?: string; nativeTurnId?: string; nativeItemId?: string; createdAt?: string; details?: NativeRequestDetails }
export type NativeAnswer = { optionId?: string; answers?: Record<string, string[]>; cancel?: boolean }
export type ContextRef = { kind: string; id: string }
export type WorkspaceRef = { id: string; revision: string }
export type NativeTurnOptions = { model?: string; effort?: string; permission?: string }
export function mergeNativeOptions(base: NativeTurnOptions = {}, override: NativeTurnOptions = {}): NativeTurnOptions {
  const merged = { ...base }
...
export type Scope = { profileId: string; context: ContextRef; workspace: WorkspaceRef; nativeAccessConfirmed: boolean; options?: NativeTurnOptions }
export type NativeSession = { schemaVersion: typeof NATIVE_SCHEMA; id: string; scope: Scope; options?: NativeTurnOptions; provider: NativeProvider; state: WorkerState; nativeSessionId?: string; createdAt: string; lastSeq: number; title: string; archived: boolean; metadataRevision: number; runtimeId?: string }
export type NativeConversationMetadata = Pick<NativeSession, 'title' | 'archived' | 'metadataRevision'>
export type NativeDecision = { outcome: 'allow' | 'deny' | 'cancel' | 'answer'; optionId?: string; optionKind?: string; actor?: { id: string; label?: string }; policyId?: string; policyRevision?: string }
export type NativeDecisionRecord = { request: NativeRequest; turnId: string; status: 'pending' | 'submitted' | 'answered' | 'expired'; decision?: NativeDecision; updatedAt?: string }
export type NativeProfile = { id: string; provider: NativeProvider; protocol: string; available: boolean; detail: string; reviewedVersion: string; interactiveRequests: boolean; toolMode: string }
export type QueuedPrompt = { id: string; text: string; options: NativeTurnOptions; requestOptions: NativeTurnOptions; createdAt: string }
export type PromptQueue = { revision: number; paused: boolean; items: QueuedPrompt[] }
export type NativeEvent = {
  queue?: PromptQueue
  schemaVersion: typeof NATIVE_SCHEMA; sessionId: string; seq: number; provider: NativeProvider
  ...
  sourceMethod?: string; nativeSessionId?: string; request?: NativeRequest
  createdAt?: string; nativeThreadId?: string; nativeTurnId?: string; details?: NativeItemDetails
  decision?: NativeDecision
  runtimeId?: string; metadata?: NativeConversationMetadata
}
export type NativeItem = { key: string; turnId: string; id: string; role: Role; text: string; title: string; status: string; truncated: boolean; turnStatus?: NativeTurnStatus; sourceMethod?: string; createdAt?: string; updatedAt?: string; nativeThreadId?: string; nativeTurnId?: string; details?: NativeItemDetails }
export type StreamState = {
  queue?: PromptQueue
  options?: NativeTurnOptions
  sessionId: string; provider: NativeProvider; cursor: number; worker: WorkerState; activeTurnId: string
  nativeSessionId: string; items: NativeItem[]; pending: Record<string, NativeRequest & { submitted: boolean }>
...
const kinds: NativeEvent['kind'][] = ['prompt.queue', 'session.state', 'session.identity', 'session.metadata', 'session.runtime', 'item.delta', 'item.snapshot', 'item.patch', 'input.request', 'input.submitted', 'input.resolved', 'turn.end', 'notice']
const roles: Role[] = ['user', 'assistant', 'tool', 'plan', 'activity']
const ID = /^[A-Za-z0-9][A-Za-z0-9._-]{0,95}$/
function record(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('原生协议对象无效。')
  return value as Record<string, unknown>
}
function string(value: unknown, limit = 262144): string {
  if (typeof value !== 'string' || value.length > limit) throw new Error('原生协议文本无效或过长。')
  return value
}
function safeId(value: unknown): string { const id = string(value, 96); if (!ID.test(id)) throw new Error('原生会话标识无效。'); return id }
function boolean(value: unknown): boolean { if (typeof value !== 'boolean') throw new Error('原生协议布尔值无效。'); return value }
function integer(value: unknown): number { if (!Number.isSafeInteger(value) || (value as number) < 0) throw new Error('原生事件序号无效。'); return value as number }
function list<T>(value: unknown, decode: (v: unknown) => T, max: number): T[] {
  if (!Array.isArray(value) || value.length > max) throw new Error('原生协议集合无效。')
  return value.map(decode)
}
function provider(value: unknown): NativeProvider { if (!PROVIDERS.includes(value as NativeProvider)) throw new Error('未知原生 Agent。'); return value as NativeProvider }
...
function nativeTimestamp(value: unknown): string {
  ...
  if (!Number.isFinite(Date.parse(result))) throw new Error('原生事件时间无效。')
  return result
}
function nativeNumber(value: unknown, nonnegative = true): number {
  if (!Number.isSafeInteger(value) || (nonnegative && (value as number) < 0)) throw new Error('原生结构化数值无效。')
...
function nativeJson(value: unknown, budget = { nodes: 0 }, depth = 0): NativeJson {
...
function nativeDetails(value: unknown): NativeItemDetails {
  ...
  switch (d.type) {
    case 'userMessage': {
      if (Object.keys(d).some(key => !['type', 'nativeOptions'].includes(key))) throw new Error('原生用户消息详情字段无效。')
      const nativeOptions = decodeNativeTurnOptions(d.nativeOptions)
      if (!Object.keys(nativeOptions).length) throw new Error('原生用户消息选项缺失。')
      return { type: d.type, nativeOptions }
    }
    case 'agentMessage': {
    ...
    case 'commandExecution':
      return { type: d.type, ...truncated, command: string(d.command, 16 * 1024), ...optionalText(d, 'cwd', 4096),
        ...
        ...(d.commandActions === undefined ? {} : { commandActions: list(d.commandActions, (entry) => {
          ...
          return { type: action.type as NativeCommandAction['type'], command: string(action.command, 16 * 1024), ...optionalText(action, 'name', 4096), ...optionalText(action, 'path', 4096), ...optionalText(action, 'query', 4096) }
        }, 64) }) }
    case 'fileChange':
      return { type: d.type, ...truncated, changes: list(d.changes, (entry) => {
        ...
        return { path: string(change.path, 4096), diff: string(change.diff, 64 * 1024), kind: { type: kind.type as NativeFileChange['kind']['type'], ...optionalText(kind, 'movePath', 4096) } }
      }, 64) }
    case 'mcpToolCall': {
      ...
      return { type: d.type, ...truncated, server: string(d.server, 4096), tool: string(d.tool, 4096),
        ...
        ...(result === undefined ? {} : { result: { content: list(result.content, (entry): NativeToolContent => {
...
function nativeRequestDetails(value: unknown): NativeRequestDetails {
...
function nativeDecision(value: unknown): NativeDecision {
  const d = record(value)
  ...
  return { outcome: d.outcome as NativeDecision['outcome'], ...optionalText(d, 'optionId', 256), ...optionalText(d, 'optionKind', 32),
...
function request(value: unknown): NativeRequest {
  ...
  return { id: safeId(r.id), kind: r.kind as NativeRequest['kind'], title: string(r.title, 4096), ...(r.text === undefined ? {} : { text: string(r.text) }), options: options(r.options), questions,
    ...optionalText(r, 'sourceMethod', 512), ...optionalText(r, 'nativeThreadId', 512), ...optionalText(r, 'nativeTurnId', 512), ...optionalText(r, 'nativeItemId', 512),
...
function nativeMetadata(value: unknown): NativeConversationMetadata {
...
export function decodeSession(value: unknown): NativeSession {
  const s = record(value), scope = record(s.scope)
  if (s.schemaVersion !== NATIVE_SCHEMA) throw new Error('原生会话版本不兼容。')
  ...
  if (boolean(scope.nativeAccessConfirmed) !== true) throw new Error('原生会话缺少明确本机授权。')
  const createdAt = string(s.createdAt, 64); if (!Number.isFinite(Date.parse(createdAt))) throw new Error('原生会话时间无效。')
  return { schemaVersion: NATIVE_SCHEMA, id: safeId(s.id), provider: provider(s.provider), state: worker(s.state), createdAt,
    ...nativeMetadata(s),
    ...(s.runtimeId === undefined ? {} : { runtimeId: safeId(s.runtimeId) }),
    ...(s.options === undefined ? {} : { options: decodeNativeTurnOptions(s.options) }),
    lastSeq: integer(s.lastSeq), ...(s.nativeSessionId === undefined ? {} : { nativeSessionId: string(s.nativeSessionId, 512) }),
    scope: { ...(scope.options === undefined ? {} : { options: decodeNativeTurnOptions(scope.options) }), profileId: safeId(scope.profileId), context: { kind: safeId(context.kind), id: safeId(context.id) }, workspace: { id: safeId(workspace.id), revision }, nativeAccessConfirmed: boolean(scope.nativeAccessConfirmed) } }
}
export function decodeSessions(value: unknown): NativeSession[] { return list(value, decodeSession, 128) }
export function decodeProfiles(value: unknown): NativeProfile[] {
...
export function decodeEvent(value: unknown, sessionId: string, expectedProvider: NativeProvider): NativeEvent {
  const e = record(value)
  if (e.schemaVersion !== NATIVE_SCHEMA || e.sessionId !== sessionId || e.provider !== expectedProvider) throw new Error('原生事件身份链不匹配，已停止显示。')
  if (!kinds.includes(e.kind as NativeEvent['kind'])) throw new Error('未知原生事件版本，已停止显示。')
  const result: NativeEvent = { schemaVersion: NATIVE_SCHEMA, sessionId, provider: expectedProvider, kind: e.kind as NativeEvent['kind'], seq: integer(e.seq) }
  if (result.seq === 0) throw new Error('原生事件序号必须从 1 开始。')
  for (const key of ['turnId', 'itemId', 'text', 'title', 'status', 'sourceMethod', 'nativeSessionId', 'nativeThreadId', 'nativeTurnId'] as const) {
  ...
  if (e.details !== undefined) {
    ...
    const legacyPrompt = e.kind === 'item.snapshot' && e.role === 'user' && e.itemId === 'user' && e.status === 'submitted'
      && Object.keys(details).length === 1 && Object.keys(details)[0] === 'nativeOptions'
  ...
  if (result.kind === 'session.identity' && (!result.nativeSessionId || result.nativeSessionId.length > 512)) throw new Error('原生会话身份缺失。')
...
export function emptyStream(sessionId: string, source: NativeProvider): StreamState {
  return { sessionId, provider: source, cursor: 0, worker: 'starting', activeTurnId: '', nativeSessionId: '', runtimeId: '', items: [], pending: {}, decisions: {}, notices: [], lastTurnStatus: '', recent: {} }
}
...
export function applyEvent(state: StreamState, raw: unknown): StreamState {
  ...
  switch (e.kind) {
    ...
    case 'session.identity':
      if (state.nativeSessionId && state.nativeSessionId !== e.nativeSessionId) throw new Error('原生会话身份发生变化。')
      next.nativeSessionId = e.nativeSessionId!; break
    case 'session.state':
    ...
    case 'item.delta': case 'item.snapshot': case 'item.patch': {
      if (e.details?.type === 'userMessage') next.options = mergeNativeOptions(state.options, e.details.nativeOptions)
      ...
      for (const key of ['nativeThreadId', 'nativeTurnId'] as const) { if (existing?.[key] && e[key] && existing[key] !== e[key]) throw new Error('同一条目的原生身份发生变化。') }
      ...
      const item: NativeItem = { key, turnId: e.turnId!, id: e.itemId!, role: e.role!, text: content.slice(0, ITEM_LIMIT), title: e.title || existing?.title || '', status: e.status || existing?.status || 'running', truncated: content.length > ITEM_LIMIT || (e.kind !== 'item.snapshot' && Boolean(existing?.truncated)),
        ...
        ...((e.nativeThreadId ?? existing?.nativeThreadId) ? { nativeThreadId: e.nativeThreadId ?? existing?.nativeThreadId } : {}),
        ...((e.nativeTurnId ?? existing?.nativeTurnId) ? { nativeTurnId: e.nativeTurnId ?? existing?.nativeTurnId } : {}),
    ...
    case 'turn.end':
      ...
      next.items = state.items.map((item) => item.turnId === e.turnId ? { ...item, turnStatus: e.status as NativeTurnStatus } : item)
