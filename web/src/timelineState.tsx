...
export type NativeTimelineState = {
  pinnedToBottom: boolean
  expandedItems: Readonly<Record<string, boolean>>
}
export const EMPTY_NATIVE_TIMELINE_STATE: NativeTimelineState = { pinnedToBottom: true, expandedItems: {} }
...
const TimelineStateContext = createContext<{
  state: NativeTimelineState
  change: (state: NativeTimelineState) => void
...
export function NativeTimelineStateProvider({ state: controlled, onChange, children }: {
  state?: NativeTimelineState; onChange?: (state: NativeTimelineState) => void; children: ReactNode
}) {
  const [local, setLocal] = useState(EMPTY_NATIVE_TIMELINE_STATE)
  const state = controlled ?? local
  const change = useCallback((next: NativeTimelineState) => {
