...
import type { AgentPermissionOption, AgentValueOption } from '../../providerSettings.js';
...
export function ComposerPermissionControl({ profileId, options, value, disabled, onChange, locale = 'en', identityId }: {
  identityId?: string; locale?: 'en' | 'zh'; profileId: string; options: readonly AgentPermissionOption[]; value?: string; disabled?: boolean; onChange: (value: string) => void;
}) {
  ...
  return <Tooltip><Select open={open && !disabled} onOpenChange={value => setOpen(!disabled && value)} value={selected?.id ?? null}
    ...
    <TooltipTrigger render={<ComposerSelectControl type="button" disabled={disabled} aria-label={locale === 'zh' ? '执行权限' : 'Permissions'} aria-description={label} data-xgc-role="agent-composer-permission" data-xgc-id={identityId ? `${identityId}:${profileId}` : profileId} className="min-w-0 shrink-0 font-medium" />}>
    ...
    <SelectPopup align="start" side="top" alignItemWithTrigger={false}>
      {options.map(option => { const OptionIcon = permissionIcons[option.id as keyof typeof permissionIcons] ?? LockIcon;
        return <SelectItem key={option.id} value={option.id} hideIndicator className="min-w-64 py-2" data-xgc-role="agent-composer-permission-option" data-xgc-id={`${identityId ? `${identityId}:` : ''}${profileId}:${option.id}`}>
...
export function ComposerEffortControl({ profileId, options, value, disabled, onChange, locale = 'en', identityId }: {
  identityId?: string; locale?: 'en' | 'zh'; profileId: string; options: readonly AgentValueOption[]; value?: string; disabled?: boolean; onChange: (value: string) => void;
}) {
  const [open, setOpen] = useState(false);
  const selected = options.find(option => option.id === value);
  return <Menu open={open && !disabled} onOpenChange={value => setOpen(!disabled && value)}>
    <MenuTrigger render={<ComposerControl type="button" disabled={disabled} aria-label={locale === 'zh' ? '思考强度' : 'Thinking effort'} data-xgc-role="agent-composer-effort" data-xgc-id={identityId ? `${identityId}:${profileId}` : profileId}
    ...
    <MenuPopup align="start" side="top"><MenuGroup><MenuGroupLabel>{locale === 'zh' ? '思考强度' : 'Thinking effort'}</MenuGroupLabel>
      <MenuRadioGroup value={selected?.id ?? ''} onValueChange={value => { if (!disabled && options.some(option => option.id === value)) onChange(value); }}>
        {options.map(option => <MenuRadioItem key={option.id} value={option.id} closeOnClick data-xgc-role="agent-composer-effort-option" data-xgc-id={`${identityId ? `${identityId}:` : ''}${profileId}:${option.id}`}>
