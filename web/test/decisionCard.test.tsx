...
import { NativeConversation, NativeInput } from '../src/NativeConversation.js'
import { applyEvent, emptyStream, NATIVE_SCHEMA, type NativeEvent, type NativeRequest } from '../src/state.js'
...
const request: NativeRequest = { id: 'review', kind: 'permission', title: 'Edit', sourceMethod: 'claude:can_use_tool',
...
function event(seq: number, patch: Partial<NativeEvent>): NativeEvent {
  return { schemaVersion: NATIVE_SCHEMA, sessionId: 'conversation', provider: 'claude', seq, turnId: 'turn', itemId: request.id,
...
describe('shared decision surface', () => {
  ...
  it('reviews exact Claude changes in the notification without a tool timeline', async () => {
    ...
    const view = render(<NativeInput sessionId="conversation" request={request} submitted={false} onAnswer={onAnswer} />)
  ...
  it('marks incomplete previews and keeps questions out of permission history', () => {
    const view = render(<NativeInput sessionId="conversation" request={{ ...request, details: { ...request.details, truncated: true } }} submitted={false} onAnswer={async () => undefined} />)
    expect(view.container.querySelector('[data-approval-detail="partial"]')?.textContent).toContain('Preview truncated')
    const question: NativeRequest = { id: 'question', kind: 'question', title: 'Choose a run', options: [], questions: [{ id: 'run', text: 'Which run?', multiple: false, freeText: true, options: [] }] }
