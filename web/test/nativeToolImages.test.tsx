...
import { AgentToolImages } from '../src/AgentToolImages.js'
...
const event={schemaVersion:AGENT_RUNTIME_SCHEMA,sessionId:'s_view',provider:'codex',seq:1,kind:'item.snapshot',turnId:'t_view',itemId:'image-one',role:'tool',status:'completed',text:'',title:'xgc2_view',details:{type:'mcpToolCall',server:'xgc2',tool:'xgc2_view',result:{content:[
...
describe('MCP image in conversation',()=>{
 it('decodes replay, maps and renders the exact received image without a new capture',()=>{
  ...
  render(<AgentToolImages images={images} identity={item.id}/>)
  ...
  expect(document.querySelector('[data-xgc-role="agent-tool-thumbnail"]')).not.toBeNull()
 })
