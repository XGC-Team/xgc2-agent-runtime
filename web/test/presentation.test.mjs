...
import { NativeConversation, NativeInput } from '../dist/react.js'
import { nativeApprovalAnswer, nativeConversationModel } from '../dist/nativePresentation.js'
...
const event = (seq, extra) => ({ schemaVersion: NATIVE_SCHEMA, sessionId: 's_test', provider: 'codex', seq, turnId: 't_test', ...extra })
const requested = () => applyEvent(emptyStream('s_test', 'codex'), event(1, { kind: 'input.request', itemId: permission.id, request: permission }))
const controlTags = html => Array.from(html.matchAll(/<button\b[^>]*data-xgc-role="native-agent-(?:approval-option|request-cancel)"[^>]*>/g), match => match[0])
const optionIds = html => Array.from(html.matchAll(/data-xgc-role="native-agent-approval-option"[^>]*data-xgc-id="([^"]+)"/g), match => match[1])
...
test('rendering a native decision never answers it and exposes stable operator controls', () => {
  let called = false
  const html = renderToStaticMarkup(createElement(NativeInput, { sessionId: 's_test', request: permission, submitted: false, locale: 'en', onAnswer: async () => { called = true } }))
  ...
  assert.doesNotMatch(html, /acceptForSession|Always allow|native-agent-composer-input|native-agent-send/)
...
test('submitted answers remain pending until the native stream resolves them', () => {
  ...
  const html = renderToStaticMarkup(createElement(NativeInput, { sessionId: state.sessionId, request: pending, submitted: pending.submitted, onAnswer: async () => { throw new Error('must not run') } }))
  assert.match(html, /aria-busy="true"/)
  assert.deepEqual(optionIds(html), ['notification:s_test:q_1:q_1:accept', 'notification:s_test:q_1:q_1:decline'])
  assert.equal(controlTags(html).length, 3)
  assert.ok(controlTags(html).every(tag => tag.includes('disabled=""')))
  assert.doesNotMatch(html, /native-agent-composer-input|native-agent-send|native-agent-interrupt/)
...
test('shared conversation projects received text once and scopes item and request identity to its session', () => {
  ...
  let called = false
  const html = renderToStaticMarkup(createElement(NativeConversation, { state, locale: 'en', onAnswer: async () => { called = true } }))
  ...
  assert.match(html, /data-xgc-role="native-agent-conversation" data-xgc-id="s_test"/)
